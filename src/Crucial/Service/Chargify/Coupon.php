...
class Coupon extends AbstractEntity
{
     ...
     * @todo Unit test should return empty array if coupon is not found (404)
     */
    public function validate($productFamilyId, $couponCode)
    {
        $service       = $this->getService();
        $response      = $service->request('product_families/' . $productFamilyId . '/coupons/validate?code=' . $couponCode, 'GET', NULL, $this->_params);
        $responseArray = $this->getResponseArray($response);
